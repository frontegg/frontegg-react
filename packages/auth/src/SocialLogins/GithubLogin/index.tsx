...
const GithubLogin: FC = (props) => {
  ...
  const redirectUrl: string | null = useRedirectUrl(createGithubUrl, SocialLoginsProvidersEnum.Github)

  const defaultButton = <SocialLoginButton name={SocialLoginsProvidersEnum.Github} action={action}>
    <GithubIcon/>
  </SocialLoginButton>

  if( redirectUrl ){
    return <div onClick={() => FronteggContext.onRedirectTo(redirectUrl, { refresh: true })}>
