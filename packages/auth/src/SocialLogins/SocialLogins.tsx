...
export const SocialLoginsContext = createContext<ISocialLoginsContext>({ action: 'Login' });
...
export const SocialLogins: SocialLoginsWithCompoundComponents = (props: SocialLoginsProps) => {
  const { loadSocialLoginsConfiguration } = useAuth(stateMapper);
  ...
  return <SocialLoginsContext.Provider value={{ action: props.action }}>
    {props.children}
